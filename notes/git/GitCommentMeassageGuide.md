...
| No. | アンチパターン | 例 | 理由 | 対策 |
|---:|---|---|---|---|
| 1 | 意味が広すぎる (`update`, `変更`) | `update: 修正` | 何をしたか不明・検索できない・fix/feat 判別不可 | 具体的なタイプを使う（`fix`/`feat`/`refactor`/`chore`） |
| 2 | 複数目的を詰め込む | `fix: バグ修正と機能追加とリファクタ` | レビュー困難・revert不可・ノイズ | 1コミット＝1目的に分割 |
| 3 | 「修正」「直した」だけ | `修正` | 何を直したか不明・履歴の価値ゼロ | 何をどう直したかを短く明記 |
...
| 9 | 自動生成ファイルをコミット | `dist/`, `node_modules/`, `build/`, `.terraform/` | 履歴が肥大化・差分が読めない | .gitignoreで除外・生成物はコミットしない |
